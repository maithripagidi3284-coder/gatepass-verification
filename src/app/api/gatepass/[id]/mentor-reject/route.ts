...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ok = await mentorSetStatus(id, session.linkedId, "MENTOR_REJECTED");
  if (!ok) return NextResponse.json({ error: "Could not reject — status may have changed." }, { status: 409 });

  await logAuditEvent({
    passId: id,
    eventType: "PASS_MENTOR_REJECTED",
    actor: session.name,
    details: "Mentor rejected the request.",
    result: "SUCCESS",
  });
  return NextResponse.json({ ...pass, status: "MENTOR_REJECTED" });
}
